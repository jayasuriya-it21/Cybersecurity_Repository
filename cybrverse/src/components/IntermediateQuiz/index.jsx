...
const IntermediateQuiz = () => {
  ...
  const [quizType, setQuizType] = useState("intermediate");
  ...
  const handleCertificateClick = () => {
    navigate('/certificateintermediate');
  };
  ...
  const handleOptionClick = (option) => {
    ...
    }, 1000); // Delay to allow users to see the correct/incorrect feedback
  };
          ...
          <div className="quiz-question">
            ...
            <div className="quiz-options">
              {currentQuiz.options.map((option, index) => (
                <button
                  key={index}
                  className={`option-button ${selectedOption === option ? (option === currentQuiz.answer ? "correct" : "incorrect") : ""}`}
